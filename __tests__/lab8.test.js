describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let buttonTextJSON = await buttonText.jsonValue();
    expect(buttonTextJSON).toBe('Remove from Cart');
    await button.click();
  }, 2500);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let cartCountTextJSON = await cartCountText.jsonValue();
    expect(cartCountTextJSON).toBe('20');
  }, 10000);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    let cartCountText = await cartCount.getProperty('innerText');
    let cartCountTextJSON = await cartCountText.jsonValue();
    expect(cartCountTextJSON).toBe('0');
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let buttonText = await button.getProperty('innerText');
      let buttonTextJSON = await buttonText.jsonValue();
      expect(buttonTextJSON).toBe('Add to Cart');
    }
    let cartCount = await page.$('#cart-count');
    let cartCountText = await cartCount.getProperty('innerText');
    let cartCountTextJSON = await cartCountText.jsonValue();
